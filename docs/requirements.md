...
- RQ-009: The system shall rank numbers from least appearance to most appearance and accept an integer input `bottom-count` meaning the minimum number of ranked number rows to output (example: `10` means at least 10 ranked rows).
...
- RQ-011: The system shall export result CSV including ranked number output with frequency and percentage values generated from `bottom-count` selection.
...
- AC-010: Given multiple numbers tied at cutoff rank, output includes all numbers at that rank, so output count can be greater than `bottom-count`.
